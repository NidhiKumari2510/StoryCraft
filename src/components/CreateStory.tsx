...
import { generateStory } from "./service/geminiService";
...
try {
  console.log("🔮 Sending request with:", { subject, storyType, ageGroup, imageStyle });

  const parsed = await generateStory(subject, storyType, ageGroup, imageStyle);
  ...
  alert("Oops! Something went wrong while creating your story. Check the browser console for details.");
} finally {
...
{/* Hero Section */} <div className="text-center mb-12"> <h1 className="text-4xl md:text-6xl font-bold bg-gradient-to-r from-purple-600 via-pink-500 to-blue-500 bg-clip-text text-transparent mb-4">
Create Your Magical Story </h1> <p className="text-xl md:text-2xl text-gray-700 font-medium">
Let's bring your imagination to life with AI magic! ✨ </p> </div>
        ...
        >
          <img
            ...
            className={`w-full h-48 mx-auto mb-3 rounded-2xl border ${
              storyType === type.id ? "border-purple-400" : "border-transparent"
        ...
        >
          <img
            ...
            className={`w-full h-48 object-cover rounded-2xl border ${
              imageStyle === style.id ? "border-blue-400" : "border-transparent"
            } transition-all duration-300`}
          />
        </button>
      ))}
